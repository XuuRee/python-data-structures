...
def dot_symbol(directory):
    """ Check if given string is dot. """
    return directory == '.'


def back_symbol(directory):
    """ Check if given string contains two dots. """
    return directory == '..'


def get_directory(current, directory):
    """ Take current directory and move to the
    next directory. Return None if directory does
    not exist."""
    if not dot_symbol(directory):
        if back_symbol(directory):
            if current.parent is not None:
                current = current.parent
        else:
            current = current.subdirectories.get(directory)
            if current is None:
                return None
    return current
...
def cd(current, path):
    ...
    for directory in paths:
        current = get_directory(current, directory)
        if current is None:
            return None
    return current
...
def create_next_directory(current, directory):
    """ Create next directory. """
    next_dir = Directory()
    next_dir.parent = current
    current.subdirectories[directory] = next_dir
    return next_dir
...
def mkdirp(current, path):
    ...
    paths = path.split('/')
    for directory in paths:
        next_dir = get_directory(current, directory)
        if next_dir is None:
            next_dir = create_next_directory(current, directory)
        current = next_dir
...
def find_recursion(result, current, pattern):
    for key, directory in current.subdirectories.items():
        if pattern in key:
            result.append(directory)
        find_recursion(result, current.subdirectories[key], pattern)
    return result
...
def find(current, pattern):
    ...
    Casova slozitost: linearni vuci velikosti podstromu aktualniho adresare
        (ale viz poznamku nahore o ignorovani slozitosti nekterych operaci)
    """
    return find_recursion([], current, pattern)
