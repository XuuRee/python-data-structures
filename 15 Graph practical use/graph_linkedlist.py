...
def isEdge(graph, u, v):
    if listFind(graph.neighbours[u], v) is None:
        return False
    return True
...
def checkLinkedList(graph, linked_list, vertex):
    node = linked_list.first
    while node is not None:
        if not graphIsValidVertex(graph, node.value):
            return False
        if not isEdge(graph, node.value, vertex):
            return False
        node = node.next
    return True
...
def isCorrect(graph):
    for i in range(graph.size):
        if not checkLinkedList(graph, graph.neighbours[i], i):
            return False
    return True
...
def insert(graph, connectTo):
    if graph.size > 0:
        index = graph.size
        linked_list = graph.neighbours[index]
        for vertex in connectTo:
            if vertex != index:
                listInsert(linked_list, vertex)
        for i in range(graph.size):
            if i in connectTo and i != index:
                listInsert(graph.neighbours[i], index)
        graph.size += 1
...
def delete(graph, vertex):
    if graph.size > 0:
        listFree(graph.neighbours[vertex])
        for i in range(graph.size):
            node = listFind(graph.neighbours[i], vertex)
            if node is not None:
                listDelete(graph.neighbours[i], node)
...
def generateTransitionMatrix(graph, matrix):
    cleanMatrix(matrix)
    for i in range(graph.size):
        linked_list = graph.neighbours[i]
        node = linked_list.first
        while node is not None:
            matrix[i][node.value] = True
            node = node.next
...
def detectCycleRecursively(graph, parent, vertex, contains, visited, unvisited):
    linked_list = graph.neighbours[vertex]
    node = linked_list.first
    visited.append(vertex)
    while node is not None:
        l_list = graph.neighbours[node.value]
        if node.value in visited and node.value != parent:
            contains.cycle = True
            return
        if node.value not in visited:
            detectCycleRecursively(graph, vertex, node.value, contains, visited, unvisited)
        node = node.next
...
def detectCycle(graph):
    contains = GraphCycle()
    visited, unvisited = [], [x for x in range(graph.size)]
    while unvisited:
        vertex = unvisited[0]
        detectCycleRecursively(graph, -1, vertex, contains, visited, unvisited)
        if contains.cycle:
            return True
    return False
