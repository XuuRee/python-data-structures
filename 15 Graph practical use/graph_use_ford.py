...
def createGraph(n):
	matrix = [[sys.maxsize]*n for i in range(n)]
	for i in range(n):
		matrix[i][i] = 0
	return matrix
	...
	for i in range(len(matrix)):
		for u in range(len(matrix)):
			for v in range(len(matrix)):
				if(matrix[u][v] == sys.maxsize): pass
				if(distance[u] + matrix[u][v] < distance[v]):
					if(distance[u] == sys.maxsize): pass
...
def pathLength(matrix, path):
    ...
    while second is not None:
        subdistance = BellmanFord(matrix, first.vertex, second.vertex)
        if subdistance == sys.maxsize:
            return sys.maxsize
...
#Test 1.
print("Test 1.: ")
test(g, None, sys.maxsize)#prazdna cesta
#Test 2.
print("Test 2.: ")
...
addEdge(g2, 1, 0, -1)
p = createPath([0,1])
test(g2, p, sys.maxsize)#zaporny cyklus v grafe
p = createPath([0,1,0])
test(g2, p, sys.maxsize)#zaporny cyklus v grafe
#Test 6.
print("Test 6.: ")
...
test(g4, p, 1)
p = createPath([1,0])
test(g4, p, sys.maxsize) #cesta neexistuje
p = createPath([2,4])
test(g4, p, 2)
p = createPath([4,2])
test(g4, p, sys.maxsize) #cesta neexistuje
