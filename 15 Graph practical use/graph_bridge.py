...
def right_edge(matrix, u, v):
	return matrix[u][v] == 1 and matrix[v][u] == 1
...
def count_components(matrix):
	components = 0
	unvisited = set([x for x in range(len(matrix))])
	while unvisited:
		visited = bfs(matrix, unvisited.pop())
		unvisited -= visited	# not good complexity
		components += 1
	return components
...
def is_bridge(matrix, u, v):
	if not right_edge(matrix, u, v):
		return False
	before = count_components(matrix)
	remove_edge(matrix, u, v)
	after = count_components(matrix)
	add_edge(matrix, u, v)
	return before != after
